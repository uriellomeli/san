...
# Change C:/Users/gpe637/Downloads for the correct path
san <- read_csv("C:/Users/gpe637/Downloads/San+Antonio+Energy+and+Time+Use+Survey+-+Spanish_March+4,+2021_14.32.csv", col_names = T)
san <- san[-c(1:9),] # delete responses before 22.02. These were responses from us. 912 responses.
```

```{r recode, include=FALSE}
san <- san %>% 
  filter(consent == "1" | disagree == "1", zipcode != "3") %>% # consent Agree and double check on survey participation. Keep only zip codes 78202 or 78230.
  mutate(duration = round(as.numeric(`Duration (in seconds)`)/60, 1), # duration in minutes
         ...
         race2 = case_when(race %in% "1" ~ "White", # recode race
                           ...
                           race %in% c("6", "7", "8") ~ "Other"), # Two or more races; Race and Ethnicity unknown|Don't know; Prefer not to answer
...
length(unique(san$email)) # 815 minus one person who didn't provide an email
```
...
san <- san %>% 
  ...
  mutate(anxious = if_else(mental_health_anxious == "1", "Not at all",
...
save(san, file = "C:/Users/gpe637/Desktop/san_energy_survey.RData")
```

```{r, message=FALSE, warning=FALSE}
load("C:/Users/gpe637/Desktop/san_energy_survey.RData")
...
san %>% 
  ...
  facet_grid(~zip) +
  ggtitle("Responses by zip code and health status") +
  xlab(NULL) +
  theme_light() +
...
san %>% 
  filter(pre_c19_ph != "NA", pre_c19_mh != "NA") %>% 
...
san %>% 
  filter(front != "NA", gender != "Other", work_hrs > 0) %>% 
  ggplot() +
  geom_col(aes(x = work_hrs, y = race_eth, color = gender, fill = gender), position = "dodge", width = .5) +
  scale_color_viridis_d(option = "E", begin = .2, end = .8, direction = -1) +
  scale_fill_viridis_d(option = "E", begin = .2, end = .8, direction = -1) +
  facet_grid(~ zip) +
...
tab22 <- san2 %>% 
  group_by(zip, rep_email) %>% 
  dplyr::summarise(av.duration = round(mean(duration, na.rm = T), 1), sd.duration = round(sd(duration, na.rm = T), 1))


kableExtra::kable(x = list(tab21, tab22), format = "html")
```
...
```{r, message=FALSE, warning=FALSE}
san %>% 
  filter(race_eth != "NH-NHPI" & race_eth != "NH-Other") %>% 
  ggplot(aes(x = health, group = race_eth)) +
  geom_bar(aes(y = ..prop.., fill = race_eth), stat = "count") +
  geom_text(aes(label = scales::percent(..prop.., accuracy = 1), y = ..prop..), stat = "count", vjust = -.5, size = 2) +
  labs(y = "Percent", fill = "health status") +
  facet_grid(zip~race_eth) +
  scale_y_continuous(labels = scales::label_percent(accuracy = 1), limits = c(0,1)) +
  scale_fill_viridis_d(option = "B", begin = .19, end = .9) +
  ggtitle("Health status by zip code and race and ethnicity") +
  xlab(NULL) +
  theme_light() +
  theme(axis.text.x = element_text(angle = 90, size = 9), 
...
poorh %>% 
  ...
  facet_grid(~zip) +
  geom_text(aes(label = label, x = poorh), nudge_y = 0.25, size = 3.2) +
  geom_text(aes(label = gender, x = poorh), nudge_y = -0.25, size = 3.2) +
  scale_fill_viridis_d(option = "B", begin = .2, end = .8, direction = 1) +
  scale_color_viridis_d(option = "B", begin = .2, end = .8, direction = 1) +
  labs(title = "Days of poor health | Women and Men",
    subtitle = "January 2021",
    x = "Average number of days with poor physical or mental health",
    y = "Days with poor healht", 
    caption = "Source: San Antonio Energy and Time Use Survey") +
  theme_light() +
  theme(plot.title = element_text(face = "bold", size = 12), plot.subtitle = element_text(size = 11), 
        ...
        strip.background = element_rect(color = "gray20", linetype = "solid"), 
