...
san.ia <- san.ia %>% 
  rename(sleep.t = per_care_1, eat.t = per_care_2, cook.t = per_care_3, groom.t = per_care_4,
   ...
   mutate(sleep.w = if_else(sleep.w == 4, "Same time as weekdays", "Different time"),
         ...
         eat.tt = if_else((eat.t != -99 & eat.w == "Same time as weekdays") | (eat.t != -99 & eat.w == "Different time"),
                          round(7*eat.t, 1), 
                          if_else(eat.t.w != -99 | eat.w == "Same time as weekdays", round(5*eat.t + 2*eat.t.w, 1), NA_real_)))
```

```{r}
summary(san.ia$eat.tt)
...
san.ia %>% 
  group_by(zipcode) %>% 
  summarise(eating_time = round(mean(eat.tt, na.rm = T), 3))
