...
themeuf <- theme(legend.position = "top", 
                 ...
                 legend.title = element_text(color = "gray30", face = "bold", size = 20), 
                 panel.background = element_rect(colour = "black"),
                 plot.title = element_text(face = "bold", size = 32, hjust = .5, vjust = .5, lineheight = .5), 
                 plot.caption = element_text(size = 18),
                 plot.subtitle = element_text(colour = "gray40", face = "bold", size = 26, lineheight = .5, hjust = .5, vjust = .5),
                 axis.text = element_text(size = 22, color = "gray20"),
                 strip.text = element_text(size = 22), 
                 axis.title.x = element_text(color = "gray20", size = 24), 
...
frontline_pct %>%
  ...
  geom_text_repel(aes(label = lbl), size = 7, color = "gray20", segment.alpha = 0, family = "foo", nudge_y = .3, nudge_x = .01) +
...
welfare %>%
  ...
  geom_text_repel(aes(label = round(average_welfare, 1)), 
                  size = 7, color = "gray20", segment.alpha = 0, family = "foo", nudge_y = .3, nudge_x = .01) +
  scale_x_continuous(breaks = seq(1, 2, .2), limits = c(1.4, 2)) +
  scale_color_manual(values = wes_palette("Darjeeling2", n = 3)) +
  guides(color = guide_legend(label.position = "left", label.vjust = 0.5, label.hjust = 1, override.aes = list(size = 5)),
         size = FALSE) +
  labs(y = NULL, x = NULL, color = NULL,
...
hardships %>% 
  ...
  geom_errorbar(aes(xmin = lower, xmax = upper), show.legend = F, width = 0.15) +
  geom_point(stat = "identity", aes(size = n)) +
  geom_text_repel(aes(label = lbl), size = 7, color = "gray20", segment.alpha = 0, family = "foo", nudge_y = .3) +
  scale_x_continuous(limits = c(0, .45), breaks = seq(0, 1, .15), label = scales::percent) +
  scale_color_manual(values = wes_palette("Darjeeling2", n = 3)) +
  guides(color = guide_legend(label.position = "left", label.vjust = 0.5, label.hjust = 1, override.aes = list(size = 5)), 
         size = F) +
...
hardships %>% 
  ...
  guides(color = guide_legend(label.position = "left", label.vjust = 0.5, label.hjust = 1, override.aes = list(size = 5)), 
         size = F) +
  labs(y = NULL, x = NULL, color = NULL,
       title = "Figure 5. Type of hardship experienced by zip code", 
...
bad %>%
  ...
  geom_errorbar(aes(xmin = lower, xmax = upper), show.legend = F, width = 0.15) +
  geom_point(stat = "identity", aes(size = n)) +
  geom_text_repel(aes(label = lbl), size = 7, color = "gray20", segment.alpha = 0, family = "foo", nudge_y = .2, nudge_x = .03) +
  scale_x_continuous(limits = c(0, .7), breaks = seq(0, 1, .15), label = scales::percent) +
  scale_color_manual(values = wes_palette("Darjeeling2", n = 3)) +
  guides(color = guide_legend(label.position = "left", label.vjust = 0.5, label.hjust = 1, override.aes = list(size = 5)), 
         size = F) +
  labs(y = NULL, x = NULL, color = NULL,
       title = "Figure 6. Number of hardships by zip code", 
