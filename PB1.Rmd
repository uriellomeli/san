...
hardships.r %>% 
  ...
  labs(y = NULL, x = NULL, color = NULL,
       ...
       caption = NULL) +
  theme_minimal() +
  themeuf + theme(legend.position = "bottom", strip.text = element_text(size = 27))
...
```{r ave_welfare_plot, message=FALSE, warning=FALSE, include=FALSE}
...
data$mental_health_anxious <- ordered(data$mental_health_anxious, levels = c("Not at all", "Several days",
                                                                             "More than half the days", "Nearly every day"))
data$mental_health_worry <- ordered(data$mental_health_worry, levels = c("Not at all", "Several days",
                                                                         "More than half the days", "Nearly every day"))
data$mental_health_interest <- ordered(data$mental_health_interest, levels = c("Not at all", "Several days",
                                                                         "More than half the days", "Nearly every day"))
data$mental_health_down <- ordered(data$mental_health_down, levels = c("Not at all", "Several days",
                                                                         "More than half the days", "Nearly every day"))

ctab1 <- data %>% 
  ggtable(columnsX = c("zipcode"), 
          columnsY = c("race_eth3", "marital_st", "ageg", "depen", "num_hsh"), 
          cells = "col.prop") +
  theme(text = element_text(size = 22))

ctab2 <- data %>% 
  #filter(!is.na(impairment)) %>% 
  ggtable(columnsX = c("race_eth3"), 
          columnsY = c("marital_st", "ageg", "depen", "num_hsh", "health", "impairment"), 
          cells = "col.prop")

ctab3 <- data %>% 
  filter(!is.na(pre_mental_health), !is.na(pre_phy_health), !is.na(mental_health_anxious), !is.na(mental_health_worry),
         !is.na(mental_health_interest), !is.na(mental_health_down)) %>% 
  ggtable(columnsX = c("race_eth3"), 
          columnsY = c("pre_mental_health", "pre_phy_health", "mental_health_anxious", "mental_health_worry",
                       "mental_health_interest", "mental_health_down"), 
          cells = "col.prop")

ctab4 <- data %>% 
  filter(!is.na(frontline), emp_status != "NA") %>% 
  ggtable("race_eth3", 
          c("frontline", "emp_status"),
          cells = "col.prop")
```
...
data %>% 
  filter(!is.na(frontline), emp_status!= "NA") %>% 
  group_by(race_eth3) %>% 
  freq(var = frontline, round.digits = 1, cumul = F)

data %>% 
  filter(!is.na(frontline), emp_status!= "NA") %>% 
  group_by(race_eth3) %>% 
  freq(var = emp_status, round.digits = 1, cumul = F)

data %>% 
  group_by(race_eth3) %>% 
  freq(var = marital_st, round.digits = 1, cumul = F)

```

<h2> Figure 1 w/o retired </h2>
```{r}
tab.jobloss.ret <- data %>%
  filter(emp_status != "retired", emp_status != "NA", !is.na(hardships_1)) %>% 
  group_by(zipcode, hardships_1, ageg) %>% 
  summarize(n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1)) 
...
tab.a <- data %>%
  filter(frontline != "NA") %>% 
  group_by(zipcode, race_eth3, frontline) %>% 
  summarize(median.age = median(age, na.rm = T),
            n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1))
...
tab.b <- data %>%
  filter(frontline != "NA") %>% 
  group_by(race_eth3, frontline) %>% 
  summarize(n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1))

tab.c0 <- data %>%
  filter(!is.na(hardships_1), hardships_1 == 1) %>% 
  group_by(race_eth3) %>% 
  summarize(n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1))
...
tab.c <- data %>%
  filter(frontline != "NA", !is.na(hardships_1), hardships_1 == 1) %>% 
  group_by(race_eth3) %>% 
  summarize(n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1))
...
tab.d <- data %>%
  filter(!is.na(frontline), !is.na(hardships_1)) %>% 
  mutate(recover = if_else(hardships_1 == 1 & emp_status == "employed", "recovered", 
                           if_else(hardships_1 == 1 & emp_status == "unemployed", "no recovered", 
                                   if_else(hardships_1 == 0 & emp_status == "employed", "never unemployed",
                                           if_else(hardships_1 == 0 & emp_status == "unemployed", "recent unemployed", NA_character_)))))%>% 
  filter(!is.na(recover)) %>% 
  group_by(race_eth3, recover) %>% 
  summarise(n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1))
  ...
  mutate(recover = if_else(hardships_1 == 1 & emp_status == "employed", "recovered", 
                           if_else(hardships_1 == 1 & emp_status == "unemployed", "no recovered", 
                                   if_else(hardships_1 == 0 & emp_status == "employed", "never unemployed",
                                           if_else(hardships_1 == 0 & emp_status == "unemployed", "recent unemployed", NA_character_)))))%>% 
  filter(!is.na(recover)) %>% 
  group_by(race_eth3, frontline, recover) %>% 
  summarise(n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1))

write.csv(tab.e,"~/GitHub/saetus/tab.e.csv", row.names = FALSE)
```

```{r split}
split.data <- split(data, data$race_eth3)

hispanics <- split.data[[1]]
whites <- split.data[[2]]
others <- split.data[[3]]

hispanics %>% 
  filter(!is.na(frontline)) %>% 
  group_by(zipcode, frontline) %>% 
  summarise(median.age = median(age, na.rm = T))

hispanics %>% 
  filter(!is.na(frontline), !is.na(hardships_1)) %>% 
  mutate(recover = if_else(hardships_1 == 1 & frontline == "Yes" & emp_status == "employed", 
                           "recover", "no recover")) %>% 
  select(zipcode, race_eth3, frontline, recover) %>% 
  group_by(race_eth3, frontline, recover) %>% 
  summarise(n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1))
...
mydata <- data %>%
  mutate(emp_status2 = if_else(emp_status %in% c("Work for pay at a job"), "employed",
                                if_else(emp_status %in% c("Temporarily absent from job", "Layoff from a job", "Actively looking for a job"), "unemployed",
...
mydata$emp_status_partner3 <- if_else(mydata$emp_status_partner %in% c("Work for pay at a job"), "employed",
                                      if_else(mydata$emp_status_partner %in% c("Temporarily absent from job", "Layoff from a job", "Actively looking for a job"), "unemployed",
...
mydata <- mydata[which(mydata$emp_status2 != "retired" & mydata$emp_status2 != "NA" & mydata$emp_status_partner3 != "retired" & mydata$emp_status_partner3 != "NA"), ]

total <- mydata[which(mydata$hardships_1 == 1),] # job loss since 2020
...
n_recover_fw <- data.frame(n_recover_fw)

colnames(n_recover_fw) <- c("Zipcode","Race","Frontline", "Total")
...
unrecover_fw <- unrecover %>% 
  filter(!is.na(frontline)) %>% 
  group_by(zipcode, race_eth3, frontline) %>% 
  summarize(n = n())

unrecover_fw[12, ] <- c("Elm Creek", "Non-Hispanic Other", "Yes", 0)

colnames(unrecover_fw) <- c("Zipcode","Race", "Frontline", "Unrecovered")

unrecover_fw <- data.frame(unrecover_fw)

unrecover_fw <- unrecover_fw %>% arrange(Zipcode)

n_recover_fw <- cbind(n_recover_fw, unrecover_fw$Unrecover)

colnames(n_recover_fw) <- c("Zipcode","Race", "Frontline", "Total", "Unrecover")

###########
### pct ###
###########
n_recover_fw <- n_recover_fw %>%
  group_by(factor(Zipcode), factor(Race)) %>% 
  mutate(pct = as.numeric(Unrecover)/as.numeric(Total),
         lbl = scales::percent(pct, accuracy = 0.1))
...
n_recover %>%
  ggplot(aes(x = pct, y = reorder(Race2,-pct), color = Zipcode)) + 
  geom_errorbar(aes(xmin = lower, xmax = upper), show.legend = F, width = 0.15) +
  geom_point(aes(size = Total)) +
  geom_text_repel(aes(label = lbl), size = 3, color = "gray10", segment.alpha = 0, family = "foo", nudge_y = .3, nudge_x = .01) +
  scale_x_continuous(limits = c(0, .75), breaks = seq(0, 1, .15), label = scales::percent) +
