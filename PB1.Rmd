...
```{r moe_race_hards, message=FALSE, warning=FALSE}
...
hardships.r %>% 
  filter(hardships != "No difficulty", 
         (hardships=="Affording food"|hardships=="Paying rent/mortgage"|hardships=="Delay getting medical care"|hardships=="Job loss"))%>% 
  ggplot(aes(x = pct, y = reorder(race_eth3, pct), color = zipcode)) +
  geom_errorbar(aes(xmin = lower, xmax = upper), show.legend = F, width = 0.1) +
  geom_point(stat = "identity", aes(size = n)) +
  geom_text_repel(aes(label = lbl), size = 7, color = "gray10", segment.alpha = 0, family = "foo", nudge_y = .25, nudge_x = .025) +
  scale_x_continuous(limits = c(0, .65), breaks = seq(0, 1, .15), label = scales::percent) +
  scale_color_manual(values = wes_palette("Darjeeling2", n = 3)) +
  guides(color = guide_legend(label.position = "left", label.vjust = 0.5, label.hjust = 1, override.aes = list(size = 5)), 
         size = F) +
  facet_wrap(~hardships, labeller = labeller(hardships = hardships_lbl), shrink = T) +
  labs(y = NULL, x = NULL, color = NULL,
...
ggsave(filename = "figure_3_newnew.png", plot = last_plot(), path = "~/GitHub/saetus/", width = 6, height = 6, units = "in", dpi = "print")
