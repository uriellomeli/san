...
jobrecovery <- data %>% 
  filter(emp_status != "Unable to work" | emp_status != "Retired") %>% 
  mutate(job_recovery = if_else(hardships_1 == 1 & emp_status == "Work for pay at a job", "Job recovery", 
                                if_else(hardships_1 == 1 & 
                                          emp_status == "Layoff from a job" | 
                                          emp_status == "Temporarily absent from job" |
                                          emp_status == "Actively looking for a job", 
                                        "No job recovery", NA_character_))) %>% 
  select(zipcode, emp_status, hardships_1, age:job_recovery) 

job.tab <- jobrecovery %>% 
  filter(!is.na(job_recovery)) %>% 
  group_by(zipcode, race_eth3, job_recovery) %>% 
  summarise(n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1)) 

job.tab2 <- jobrecovery %>% 
  filter(!is.na(job_recovery)) %>% 
  group_by(zipcode, race_eth3, job_recovery) %>% 
  summarise(n = n()) %>% 
  mutate(pct = n/sum(n),
...
job.tab %>% 
  filter(job_recovery == "No job recovery")
```
...
write.csv(tab.jobloss.ret,"~/GitHub/saetus/table_jobloss.csv", row.names = FALSE)
```

```{r}
mydata$emp_recover = mydata$hardships_1==1 & mydata$emp_status=="employed" & mydata$emp_status_partner=="employed"

data.check <- data %>%
  filter(emp_status != "NA", emp_status != "retired", emp_status_partner != "NA", emp_status_partner != "retired") %>% 
  mutate_at(vars(emp_status_partner), ~if_else(. %in% c("Work for pay at a job","Temporarily absent from job"), "employed",
                                    if_else(. %in% c("Layoff from a job","Actively looking for a job"), "unemployed",
                                            if_else(. %in% c("Retired", "Unable to work"), "retired", "NA")))) %>% 
  mutate(emp_recover = if_else(hardships_1 == 1 & emp_status == "employed" & emp_status_partner == "employed", "Recover", 
                               "No recover")) %>% 
  filter(emp_recover != "NA") %>% 
  group_by(zipcode, emp_recover, race_eth3) %>% 
  summarize(n = n()) %>% 
  mutate(pct = n/sum(n),
         lbl = scales::percent(pct, accuracy = 0.1)) %>% 
  ungroup() %>% 
  filter(emp_recover == "Recover")
```
