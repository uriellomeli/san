...
```{r loss_income_plot, warning=FALSE, message=FALSE, include=FALSE}
fig3 <- loss %>% 
  ggplot(aes(x = pct, y = reorder(race_eth3, pct), color = zipcode)) +
  geom_errorbar(aes(xmin = lower, xmax = upper), show.legend = F, width = 0.1) +
...
```{r plot_figure_2, message=FALSE, warning=FALSE, include=FALSE}
fig5
```

```{r plot_figure_3, message=FALSE, warning=FALSE, include=FALSE}
# facet names
...
ctab1 <- data %>% 
  ggtable(columnsX = c("zipcode"), 
          ...
          cells = "col.prop") +
  theme(text = element_text(size = 25), axis.text = element_text(size = 30), axis.title = element_text(size = 30),
...
#ggsave(filename = "ctab1.png", plot = ctab1, path = "~/GitHub/saetus/", device = "png")
```
...
data %>% 
  filter(!is.na(frontline), emp_status!= "NA") %>% 
  group_by(race_eth3) %>% 
  freq(var = frontline, round.digits = 1, cumul = F, style = "grid")

data %>% 
  filter(!is.na(frontline), emp_status!= "NA") %>% 
  group_by(race_eth3) %>% 
  freq(var = emp_status, round.digits = 1, cumul = F, style = "grid")

data %>% 
  group_by(race_eth3) %>% 
  freq(var = marital_st, round.digits = 1, cumul = F, style = "grid")

data %>% 
  group_by(race_eth3) %>% 
  freq(var = ageg, round.digits = 1, cumul = F, style = "grid")

data %>% 
  group_by(race_eth3) %>% 
  freq(var = depen, round.digits = 1, cumul = F, style = "grid")

data %>% 
  group_by(race_eth3) %>% 
  freq(var = num_hsh, round.digits = 1, cumul = F, style = "grid")

data %>% 
  group_by(race_eth3) %>% 
  freq(var = health, round.digits = 1, cumul = F, style = "grid")

data %>% 
  filter(!is.na(impairment)) %>% 
  group_by(race_eth3) %>% 
  freq(var = impairment, round.digits = 1, cumul = F, style = "grid")

data %>% 
  filter(!is.na(pre_mental_health)) %>%
  group_by(race_eth3) %>% 
  freq(var = pre_mental_health, round.digits = 1, cumul = F, style = "grid")

data %>% 
  filter(!is.na(pre_phy_health)) %>%
  group_by(race_eth3) %>% 
  freq(var = pre_phy_health, round.digits = 1, cumul = F, style = "grid")

data %>% 
  filter(!is.na(mental_health_anxious)) %>%
  group_by(race_eth3) %>% 
  freq(var = mental_health_anxious, round.digits = 1, cumul = F, style = "grid")

data %>% 
  filter(!is.na(mental_health_worry)) %>%
  group_by(race_eth3) %>% 
  freq(var = mental_health_worry, round.digits = 1, cumul = F, style = "grid")

data %>% 
  filter(!is.na(mental_health_interest)) %>%
  group_by(race_eth3) %>% 
  freq(var = mental_health_interest, round.digits = 1, cumul = F, style = "grid")

data %>% 
  filter(!is.na(mental_health_down)) %>%
  group_by(race_eth3) %>% 
  freq(var = mental_health_down, round.digits = 1, cumul = F, style = "grid")
```

```{r}
data %>%
  group_by(race_eth3) %>%
  summarise(av.hsh.size = mean(num_hsh, na.rm = T), med.hsh.size = median(num_hsh, na.rm = T))
```
